...
#define IO_PERIOD 1000        // IO_Completion_Interrupt 가 sleep 하는 주기, ms 단위
#define NUMBER_OF_PRIORITY 3 // 다단계 피드백 큐 우선순위 개수
#define PRIORITY_FST 0
#define PRIORITY_SEC 1
#define PRIORITY_TRD 2

#define AGING_PERIOD 3000    // aging 을 위한 로직 주기
#define MAX_WAIT 3           // 현재 우선순위 큐에서 최대로 대기하는 시간

#define TIMER_RUN 1          // timer interrupt 실행
#define TIMER_PAUSE 0        // timer interrupt 멈춤
...
std::condition_variable age_cv; // aging 을 위한 조건 변수
std::mutex age_cv_m;           // aging 을 위한 뮤텍스
...
struct proc_tbl_t awake_q;  // aging 을 기다리면서 프로세스들을 집어넣는 큐
...
int main()
{
    ...
    awake_q.next = awake_q.prev = &(awake_q);
...
//    Print_Q(&run_q);
    ...
    //p->wait = 0;
    //p->state = PROCESS_READY;
    //p->th = std::thread(Aging_Interrupt, 10);
    //p->priority_level = PRIORITY_FST;
...
void IO_Completion_Interrupt(int id)
{
    ...
    for (;;) { // 무한반복하며 주기적으로 인터럽트 발생시킴
        ...
        if (p != NULL)                 // 현재 깨어나기를 기다리는 블록된 프로세스가 있다면
        {
            ...
            Put_Tail_Q(&(awake_q), p);
            while (1)
            {
                awake_p = Get_Head_Q(&(awake_q));
                if (awake_p == NULL) // 더 이상 깨어났지만 준비큐에 들어가지 못한 프로세스가 없다면, 종료
                {
                    break;
                }
                Put_Tail_Q(&(ready_q[awake_p->priority]), awake_p);      // 우선순위에 맞는 ready_q 맨 끝에 추가함
                std::cout << awake_p->id << " process wakes up and its state changes to ready state\n";
                switch (awake_p->priority_level)
                {
                case PRIORITY_FST:
                    std::cout << awake_p->id << " process' priority is now First\n";
                    break;
                case PRIORITY_SEC:
                    std::cout << awake_p->id << " process' priority is now Second\n";
                    break;
                case PRIORITY_TRD:
                    std::cout << awake_p->id << " process' priority is now Third\n";
                    break;
                default:
                    std::cout << awake_p->id << " process' priority is now First\n";
                    break;
